# GetItRight — Property Valuation & Legal Reports Marketplace
...
Run marketplace seed separately: `docker cp backend/scripts/seed_marketplace.py getitright-backend-1:/app/scripts/ && docker compose -f docker-compose.local.yml --env-file .env.local exec backend python -m scripts.seed_marketplace` — creates 4 vendor profiles with tiers, 30 localities (4 cities), and 10 price bands.
...
- **Alembic migrations from container:** Migrations generated inside Docker stay in the container (alembic/ not volume-mounted). Copy to host with: `docker cp getitright-backend-1:/app/alembic/versions/<file>.py backend/alembic/versions/`
...
- **Running new scripts:** Copy into container first: `docker cp backend/scripts/foo.py getitright-backend-1:/app/scripts/` then `docker compose -f docker-compose.local.yml --env-file .env.local exec backend python -m scripts.foo`
